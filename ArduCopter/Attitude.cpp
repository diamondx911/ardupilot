...
float Copter::get_pilot_desired_lateral(int16_t lateral_control, float thr_mid)
{
    if (thr_mid <= 0.0f) {
        thr_mid = motors->get_throttle_hover();
    }

    int16_t mid_stick = channel_lateral->get_control_mid();
    // protect against unlikely divide by zero
    if (mid_stick <= 0) {
        mid_stick = 500;
    }
    ...
    // ensure reasonable throttle values
    lateral_control = constrain_int16(lateral_control,0,1000);

    // calculate normalised throttle input
    float lateral_in;
    if (lateral_control < mid_stick) {
        // below the deadband
        lateral_in = ((float)lateral_control)*0.5f/(float)mid_stick;
    }else if(lateral_control > mid_stick) {
        // above the deadband
        lateral_in = 0.5f + ((float)(lateral_control-mid_stick)) * 0.5f / (float)(1000-mid_stick);
    }else{
        // must be in the deadband
        lateral_in = 0.5f;
    }
    ...
    float expo = constrain_float(-(thr_mid-0.5)/0.375, -0.5f, 1.0f);
    // calculate the output throttle using the given expo function
    float lateral_out = lateral_in*(1.0f-expo) + expo*lateral_in*lateral_in*lateral_in;
    return lateral_out;
}

float Copter::get_pilot_desired_forward(int16_t forward_control, float thr_mid)
{
    if (thr_mid <= 0.0f) {
        thr_mid = motors->get_throttle_hover();
    }

    int16_t mid_stick = channel_forward->get_control_mid();
    // protect against unlikely divide by zero
    if (mid_stick <= 0) {
        mid_stick = 500;
    }
    ...
    // ensure reasonable throttle values
    forward_control = constrain_int16(forward_control,0,1000);

    // calculate normalised throttle input
    float forward_in;
    if (forward_control < mid_stick) {
        // below the deadband
        forward_in = ((float)forward_control)*0.5f/(float)mid_stick;
    }else if(forward_control > mid_stick) {
        // above the deadband
        forward_in = 0.5f + ((float)(forward_control-mid_stick)) * 0.5f / (float)(1000-mid_stick);
    }else{
        // must be in the deadband
        fortward_in = 0.5f;
    }
    ...
    float expo = constrain_float(-(thr_mid-0.5)/0.375, -0.5f, 1.0f);
    // calculate the output throttle using the given expo function
    float forward_out = forward_in*(1.0f-expo) + expo*forward_in*forward_in*forward_in;
    return forward_out;
}
